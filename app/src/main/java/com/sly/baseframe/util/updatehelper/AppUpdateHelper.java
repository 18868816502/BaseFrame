...
import com.sly.baseframe.data.AppUpdateBean;
import com.sly.baseframe.dialog.CommNoneAndroidDialog;
...
//                        });
    ...
    public void processAppUpdate(AppUpdateBean appUpdate, AppCompatActivity activity) {
        weakReference = new WeakReference<>(activity);
        startDownload(activity, appUpdate.getVersionUrl(), appUpdate.getVersion(), appUpdate.getHasForcedUpgrade() == 1);
    }

    private void handleUpdate(AppUpdateBean app, WeakReference<AppCompatActivity> wr) {
        if (wr.get() != null) {
    ...
    private void startDownload(Context context, String url, String version, boolean isForce) {
        ...
        try {
            ...
            weakReference.get().runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    ToastCommonUtil.createToastConfig().normalToast(weakReference.get(), "文件读取失败，请检查应用权限");
                    //重新弹窗
                    handleUpdate(app, weakReference);
                }
            });
        ...
        protected void onPostExecute(Boolean aBoolean) {
            ...
            if (aBoolean) {
                ...
                if (helper != null && helper.weakReference.get() != null) {
                    ToastCommonUtil.createToastConfig().normalToast(helper.weakReference.get(), "下载失败，请检查网络或者应用权限");
                }
            }
        }
    }
